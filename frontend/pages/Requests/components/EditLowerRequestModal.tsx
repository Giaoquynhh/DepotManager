...
import type { EditLowerRequestModalProps, EditLowerRequestData, ExistingFile } from './EditLowerRequestModal.types';
import { ModalHeader } from './ModalHeader';
import { LowerRequestFormFields } from './LowerRequestFormFields';
import { DocumentsUploader } from './DocumentsUploader';

export const EditLowerRequestModal: React.FC<EditLowerRequestModalProps> = ({
	isOpen,
	onClose,
	onSubmit,
	requestData
}) => {
	const [formData, setFormData] = useState<EditLowerRequestData>({
		requestNo: '',
		shippingLine: '',
		containerNumber: '',
		containerType: '',
	...
	const handleInputChange = (field: keyof EditLowerRequestData, value: string) => {
		setFormData(prev => ({
			...prev,
			[field]: value
		}));
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const updateData = {
				type: requestData.type || 'IMPORT', // Giữ nguyên type của request hiện tại, mặc định là IMPORT cho trang hạ container
	...
	return (
		<div style={{
			...
			{/* Header */}
				<ModalHeader title="Chỉnh sửa yêu cầu hạ container" onClose={onClose} />
