    ...
    try {
        ...
        const request = await prisma.serviceRequest.create({
            data: {
                ...
                status: status || 'PENDING', // Sử dụng status từ request body, mặc định là PENDING
...
// Chuyển trạng thái từ PENDING sang GATE_IN
export const moveToGate = async (req: Request, res: Response) => {
    try {
        ...
        if (request.status !== 'PENDING') {
            return res.status(400).json({
                success: false,
                message: 'Chỉ có thể chuyển yêu cầu từ trạng thái PENDING sang GATE_IN'
            });
        }

        // Cập nhật trạng thái và thông tin gate
        const updatedRequest = await prisma.serviceRequest.update({
            where: { id },
            data: {
                status: 'GATE_IN',
        ...
        res.json({
            success: true,
            data: updatedRequest,
            message: 'Yêu cầu đã được chuyển đến cổng thành công'
        });
        ...
        res.status(500).json({
            success: false,
            message: error.message || 'Có lỗi xảy ra khi chuyển yêu cầu đến cổng'
        });
    }
};
