...
async function fillBookingFromRequest() {
  try {
    console.log('📝 ĐIỀN BOOKING TỪ REQUEST TƯƠNG ỨNG');
    console.log('=' .repeat(60));

    // Sử dụng file cuối cùng với ghi chú
    const templatePath = path.join(__dirname, 'uploads/generated-eir/EIR_OO11_WITH_NOTES_2025-10-03T21-47-27.xlsx');
    
    if (!fs.existsSync(templatePath)) {
      console.log('❌ File với ghi chú không tồn tại:', templatePath);
      return;
    }
    ...
    console.log(`   - Booking: "${request.booking_number || 'Không có booking'}"`);
    ...
    let filledCells = 0;
    const bookingNumber = request.booking_number || '';

    console.log('\n📝 ĐIỀN BOOKING TỪ REQUEST:');
    console.log('=' .repeat(50));

    // G9:H9 - Booking từ request
    ...
    const filename = `EIR_OO11_WITH_BOOKING_${timestamp}.xlsx`;
    ...
    console.log('✅ Phiếu EIR với booking đã được tạo thành công!');
    ...
    console.error('❌ Lỗi khi điền booking từ request:', error);
  } finally {
    await prisma.$disconnect();
  }
}

fillBookingFromRequest();
